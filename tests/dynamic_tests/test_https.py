...
def test_http_root_redirects_to_https():
    response = requests.get(f"{HTTP_BASE}/", allow_redirects=False, timeout=5, headers=headers)
    assert response.status_code in (301, 308)
    assert response.headers["Location"].startswith("https://")

def test_http_health_redirects_to_https():
    response = requests.get(f"{HTTP_BASE}/health", allow_redirects=False, timeout=5, headers=headers)
    assert response.status_code in (301, 308)
    assert response.headers["Location"].startswith("https://")
    assert response.headers["Location"].endswith("/health")

def test_https_health_is_reachable():
    response = requests.get(f"{HTTPS_BASE}/health", verify=False, timeout=5, headers=headers)
